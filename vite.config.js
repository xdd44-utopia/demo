...
// Per-quality preset: [maxAngleDeg, gridMinCount, gridMaxCount]
...
const QUALITY_PRESETS = {
  1: { maxAngle: 40, gridMin: 1,  gridMax: 4  },  // Coarse
  2: { maxAngle: 25, gridMin: 2,  gridMax: 8  },  // Low
  3: { maxAngle: 15, gridMin: 4,  gridMax: 16 },  // Medium
  4: { maxAngle:  8, gridMin: 8,  gridMax: 32 },  // High
  5: { maxAngle:  3, gridMin: 16, gridMax: 64 },  // Ultra
};

function buildScript({ maxAngle, gridMin, gridMax }) {
  return `
  ...
  plugins: [
    {
      ...
      configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
          ...
          if (pathname !== '/api/sync') return next();

          const q = Math.min(5, Math.max(1, parseInt(searchParams.get('q') || '3', 10)));
          const preset = QUALITY_PRESETS[q];
          console.log(`[rhino-api] sync quality=${q} (maxAngle=${preset.maxAngle}°, gridMin=${preset.gridMin})`);
          ...
          try {
            ...
            // result.output sometimes contains the JSON twice (rhinomcp echoes stdout)
